...
const Signup = () => {
  ...
  const validateForm = () => {
    ...
    const requiredFields = ['firstname', 'lastname', 'email', 'password', 'confirmPassword', 'phone', 'address', 'gender', 'date_of_birth', 'cnic', 'blood_group', 'occupation', 'marital_status'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(process.env.REACT_APP_API_URL + "persons/", formData);
...
const Signup = () => {
  const navigate = useNavigate();
>>>>>>> 4742922c2a8b2352b9298286ce875499f383f881
  return (
...
<<<<<<< HEAD
        ...
        {formData.image && (
          <div className="relative mt-4">
            <img src={formData.image} alt="Uploaded" className="w-40 h-40 object-cover rounded-lg" />
            <button type="button" className="absolute top-0 right-0 p-1 bg-red-500 rounded-full text-white" onClick={handleRemoveImage}>
